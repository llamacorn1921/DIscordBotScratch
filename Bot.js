...
const server = new WebSocket("wss://gateway.discord.gg/?v=9&encoding=json");
...
server.onmessage = (e) => { // every message received from server
    ...
    switch (op) { // decides what to do for what
        case 10: // first connection to server
            {
                ...
                send({ // send authentication to server
                    op: 2, // op: identify
                    d: {
...
function heartBeat () { // send heartbeat
    setInterval(() => { // repeat every x seconds
        send(1,botInfo.seq)
    }, botInfo.heartbeat);
}